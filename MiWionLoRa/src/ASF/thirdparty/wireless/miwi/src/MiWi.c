...
bool scanTimerFlag = false;
extern uint8_t scanTimerId;
//kwlee 
int MAC_ADD_CNT;

extern  bool hasAck;

uint32_t set_PACKET_RECORD_TIMEOUT;
	...
	*/
	#define SCAN_DURATION_0 1
    #define SCAN_DURATION_1 2
    #define SCAN_DURATION_2 4
    #define SCAN_DURATION_3 8
    #define SCAN_DURATION_4 16
    #define SCAN_DURATION_5 32 
    #define SCAN_DURATION_6 64
    #define SCAN_DURATION_7 128
    #define SCAN_DURATION_8 256
    #define SCAN_DURATION_9 512	
    #define SCAN_DURATION_10 1024
    #define SCAN_DURATION_11 2048
    #define SCAN_DURATION_12 4096	// 1sec
    #define SCAN_DURATION_13 8192	
    #define SCAN_DURATION_14 16384
    ...
    uint8_t            currentChannel;             // current operating channel for the device
    uint8_t            ConnMode = 0;
    typedef union
    {
        uint8_t        Val;
    ...
    #endif
        //#if (TX_BUFFER_SIZE + MIWI_PRO_HEADER_LEN) > 112     // eugene CY17.01.25
            //uint8_t TxBuffer[TX_BUFFER_SIZE];
        //#else
    ...
    // network related info into NVM
        ...
        struct
        {
            ...
            #if (TX_BUFFER_SIZE + MIWI_PRO_HEADER_LEN) > 112 // eugene CY17.01.25
                uint8_t RandomSendTxBuffer[TX_BUFFER_SIZE];
            #else
            ...
            uint8_t EnergyScanResults[32+1];
        #endif
                                            ...
                                            case OPEN_SOCKET_REQUEST:
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {                                            
                                                            //ConsolePutROMString((char*)"sending out response\r\n");
...
ASSIGN_COORDINATOR_SHORT_ADDRESS:                                                            
                            ...
                            else
                            {
                                ...
                                {
									//printf("chk3 ");
									//printf("%x ",MACRxPacket.Payload[8]);
									//printf("%x ",MACRxPacket.Payload[9]);
									//printf("%x ",MACRxPacket.Payload[10]);
									//printf("%x ",MACRxPacket.Payload[11]);
...
START_ASSOCIATION_RESPONSE:                       
                                ...
                                TxBuffer[3] = associateStatus;
									...
									printf("TEMP ADDR: 0x%4x\r\n",tempShortAddress.Val);
                        ...
                        case MAC_COMMAND_ASSOCIATION_RESPONSE:
                            {
								if(MACRxPacket.Payload[3] == ASSOCIATION_SUCCESSFUL)
                                {
									...
									//if(MAC_ADD_CNT==0){
										//myShortAddress.v[0] = MACRxPacket.Payload[1]+MAC_ADD_CNT;
										//myShortAddress.v[1] = MACRxPacket.Payload[2]+MAC_ADD_CNT;
										//MAC_ADD_CNT=1;
										////MAC_ADD_CNT=rand_r(1);
										//
									//}
									//kwlee 
									//myShortAddress.Val = MY_SHORT_ADDR;
                                    //
...
/*	
		...
		if(0 != memcmp(&EDBGDevEUI, &invalidEDBGDevEui, sizeof(EDBGDevEUI)))
		{
			
			memcpy(myLongAddress, EDBGDevEUI, sizeof(MY_ADDRESS_LENGTH));
		}
		
		//ConnectionTable read test
		unsigned char ee_read_test[130];
		i2c_EEPROM_Read(ee_read_test, 0, 128);	//ConnectionTable read test
		NVMRead(ee_read_test, 0, 128);	//ConnectionTable read test
		*/	
        #if 0	//defined(ENABLE_EEPROM_MAC_EUI)
			#if	defined(EEPROM_MAC_EUI_64)
				for(i = 0; i < MY_ADDRESS_LENGTH; i++)
				{
					//EEPROMRead(&(myLongAddress[MY_ADDRESS_LENGTH-1-i]), EEPROM_MAC_ADDR+i, 1);
					i2c_EEPROM_Read(&(myLongAddress[MY_ADDRESS_LENGTH-1-i]), (0x100-MY_ADDRESS_LENGTH)+i, 1);
				}
			#elif	defined(EEPROM_MAC_EUI_48)
				if( MY_ADDRESS_LENGTH > 6 )
				{
					for(i = 0; i < 3; i++)
					{
						//EEPROMRead(&(myLongAddress[MY_ADDRESS_LENGTH-1-i]), EEPROM_MAC_ADDR+i, 1);
						i2c_EEPROM_Read(&(myLongAddress[MY_ADDRESS_LENGTH-1-i]), EEPROM_MAC_ADDR+i, 1);
					}
					...
					for(i = 0; i < 3; i++)
					{
						...
						i2c_EEPROM_Read(&(myLongAddress[2-i]), EEPROM_MAC_ADDR+3+i, 1);
					}
				}
				else
				{
        ...
        TxData = 0;
        ...
        #if defined(ENABLE_INDIRECT_MESSAGE) && defined(NWK_ROLE_COORDINATOR)   // eugene
            for(i = 0; i < INDIRECT_MESSAGE_SIZE; i++)
            {
                indirectMessages[i].flags.Val = 0;
            }
        ...
        #if 0//defined(ENABLE_NETWORK_FREEZER)
			...
			if( bNetworkFreezer && (currentChannel >= USE_CHANNEL_NUM_START && currentChannel <= USE_CHANNEL_NUM_END) && (myShortAddress.Val != 0xFFFF))
            {
//                nvmGetCurrentChannel(&currentChannel);
...
//                }
            ...
            {
				currentChannel = INIT_CHANNEL_NUM;
...
{
    uint8_t i;
    uint32_t channelMask = 0xFFFFFFFF;//0x00000001;  
    ...
    while(i <= USE_CHANNEL_NUM_END)
    {
        if( ChannelMap & FULL_CHANNEL_MAP & (channelMask << (i-1)) )
        {
            ...
            MiApp_WriteData(i);	// == MiApp_WriteData(currentChannel);
				if(RF_OUTPUT==20){
					port_pin_set_output_level(RF_SWITCH_PIN, RF_SWITCH_INACTIVE);//kwlee 210215
				}else{
					port_pin_set_output_level(RF_SWITCH_PIN, RF_SWITCH_ACTIVE);//kwlee 210215
				}
            #if defined(IEEE_802_15_4)
			...
			port_pin_set_output_level(RF_SWITCH_PIN, RF_SWITCH_ACTIVE);//kwlee 210215 rx
			t1 = MiWi_TickGet();
			//printf("t1: %d -",t1);
            while(1)
            {					
	...
	if(search_count != 1)
	{
		//채널 1개만 검색시 채널 복원하지 않고 해당 채널로 변경.
		//printf("no scan...\r\n");
		MiApp_SetChannel(backupChannel);
	}
...
uint8_t    MiApp_EstablishConnection(INPUT uint8_t ActiveScanIndex, INPUT uint8_t Mode)
{
    ...
    if( Mode == CONN_MODE_DIRECT)
    {
        if( ActiveScanIndex == 0xFF )
        {
			...
			//printf("temp PID: %.4x",tempPANID.Val);
		...
		channel = NVM_Read_CH(); //read channel
		MiApp_SetChannel(channel);
		
		printf("\r\nStart from NVM on Channel - %d \r\n",channel);
...
bool UnicastShortAddress(uint8_t *DestAddress)
{
    ...
    #if 0// defined(NWK_ROLE_COORDINATOR)
    ...
    switch(Mode)
    {
...
#if defined(ROLE_TREE_PAN_COORDINATOR)
                role = ROLE_PAN_COORDINATOR;
#endif
#if defined(ROLE_TREE_COORDINATOR)
                role = ROLE_COORDINATOR; 
#endif
#if defined(ROLE_TREE_ENDDEVICE)
                role = ROLE_FFD_END_DEVICE;
#endif
                MiWiPROCapacityInfo.bits.Role = role;
                FamilyTree[0] = 0;
        ...
        case START_CONN_ENERGY_SCN:
            #if defined(ENABLE_ED_SCAN) && defined(NWK_ROLE_COORDINATOR)
            {
		        ...
		        printf("\r\nStart Wireless Communication on Channel - %d \r\n",channel);
				...
				#if defined(ENABLE_NETWORK_FREEZER)
					NVM_Write_CH(channel);	//save channel					
                #endif
    ...
    return false;
        ...
        uint8_t MiApp_NoiseDetection(uint32_t ChannelMap, uint8_t ScanDuration, uint8_t DetectionMode, int16_t *RSSIValue)
        {
			...
			while(i <= USE_CHANNEL_NUM_END)
            {
                ...
                {
                    /* choose appropriate channel */
                    MiApp_SetChannel(i);//kwlee 210214
